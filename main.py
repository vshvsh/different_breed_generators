...
def prepare_data(_conn):
  ...
  vehicle_chassis_dict = vehicle_chassis.set_index('Type').to_dict(orient='index')
  ...
  ground_weapons_grouped = {size: group.set_index('Name').to_dict('index') for size, group in ground_weapons_df.groupby('Size')}
  ...
  vehicle_slot_costs_dict = vehicle_slot_costs.set_index('Slot Type').to_dict(orient='index')
  ...
  vehicle_wargear_dict = vehicle_wargear.set_index('Name').to_dict(orient='index')
...
vehicle_chassis_dict, ground_weapons_grouped,   vehicle_slot_costs_dict, vehicle_slots_conversion, vehicle_wargear_dict = prepare_data(conn)
...
for stype in vehicle_slot_costs_dict:
  inp = st.number_input("Add/refund " + stype + " slots", value=0, min_value=-10, max_value=100)
  if inp>0:
    ...
    vehicle_vote += "-[x] Add " + str(inp) + " " + stype + " slots, " + str(ss_cost) + " SS\n"
  elif inp<0:
    vehicle_design[stype + ' Slots'] += inp
    ss_cost = inp*vehicle_slot_costs_dict[stype]['System Slots Refunded']
    vehicle_design['SS refunded'] -= round(ss_cost)
    vehicle_vote += "-[x] Refund " + str(inp) + " " + stype + " slots for " + str(abs(ss_cost)) + " SS\n"

#slot conversion
for stype in vehicle_slot_costs_dict:
  if vehicle_design[stype + ' Slots'] > 0 and stype != 'Ranged Weapon':
    inp = st.number_input("Downgrade " + stype + " slots", value=0, min_value=0, max_value=vehicle_design[stype + ' Slots'])
    if inp>0:
      type_order = {'Superheavy Weapon' : ('Vehicle Weapon', 3), 
                    'Vehicle Weapon' : ('Heavy Weapon', 2),
                    'Heavy Weapon' : ('Ranged Weapon', 2),
                   }
      vehicle_design[stype + ' Slots'] -= inp
      vehicle_design[type_order[stype][0] + ' Slots'] += inp*type_order[stype][1]
      vehicle_vote += "-[x] Downgrade " + str(inp) + " " + stype + " slots to " + str(type_order[stype][1]*inp) + " " + type_order[stype][0] + " slots\n" 
...
for stype in reversed(vehicle_slot_costs_dict.keys()):
  ...
  if slots > 0:
    ...
    for i in range(0, slots):
      weapon = st.selectbox("Select weapon", ground_weapons_grouped[stype].keys(), key=str(i)+stype)
      if weapon:
...
for i in range(0, wargear_cnt):
  wg = st.selectbox("Select wargear", vehicle_wargear_dict.keys(), key=str(i)+"wargear")
  amount = st.number_input("Number", value=1, min_value=1, max_value=60, key = str(i)+"wargearcnt")
  if wg:
    ...
    if wg == 'Vehicle Grav-Shield' and amount > grav_allowed:
      st.write(f"Warning! You can only have {grav_allowed} Vehicle Grav-Shield on this chassis")
...
if vehicle_design['Base EP']:
  vehicle_vote += f"-[x] Total EP cost should be around {vehicle_design['Gear EP costs'] + vehicle_design['Base EP']} EP\n"


ss_left =  vehicle_design['System Slots'] + vehicle_design['SS refunded'] -  vehicle_design['SS spent']
if ss_left<0:
  st.write(f'Warning! {vehicle_design["SS spent"]} system slot spent out of {vehicle_design["System Slots"]} base + {vehicle_design["SS refunded"]} refunded')
